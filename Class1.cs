...
namespace text
{
    class Class1
    {
        static void Main(String[] args)//底层算法测试
        {
            ...
            for (int i = 0; i < s; i++)
            {
                Console.WriteLine(text1.MakePlan(L, now, bt1)[i].ActualStartTime);
                Console.WriteLine(text1.MakePlan(L, now, bt1)[i].ActualEndTime);
                Console.WriteLine(text1.MakePlan(L, now, bt1)[i].BatchID);
                Console.WriteLine(text1.MakePlan(L, now, bt1)[i].BatchName);
                Console.WriteLine(text1.MakePlan(L, now, bt1)[i].Building);
                Console.WriteLine(text1.MakePlan(L, now, bt1)[i].ComponentID);
                Console.WriteLine(text1.MakePlan(L, now, bt1)[i].ComponentType);
                Console.WriteLine(text1.MakePlan(L, now, bt1)[i].Floor);
                Console.WriteLine(text1.MakePlan(L, now, bt1)[i].FlowLineNumber);
                Console.WriteLine(text1.MakePlan(L, now, bt1)[i].ProjectName);
                Console.WriteLine();
            }
            Console.WriteLine(s);
            Console.ReadLine();
            ...
            for (int i = 0; i < pi; i++)
            {
                ...
                for (int j = 0; j < bi; j++)
                {
                    ...
                    for (int downf = 1; downf <= maxfloor; downf++)//简单易懂
                    {
                        ...
                        gp = CoreFunc(GJ, PP, L, starttime);              //底层算法，获得生产排序
                        DateTime time = starttime;
                        for (int k = 0; k < gp[0]; k++) //输出
                        {
                            //生成planobject
                            ...
                            plan.ComponentType = ("外墙");
                            plan.ComponentID = k.ToString();
                            plan.FlowLineNumber = ("外墙生产线");
                            DateTime d = time;
                            time = time.AddHours(gp[8]);
                            plan.ActualStartTime = d;
                            plan.ActualEndTime = time;
                            list.Add(plan);
                        }
                        ...
                        for (int k = 0; k < gp[4]; k++)
                        {
                            ...
                            plan.FlowLineNumber = ("固定模台生产线");
                            DateTime d = time;
                            time = time.AddHours(gp[12]);
                            plan.ActualStartTime = d;
                            plan.ActualEndTime = time;
                            list.Add(plan);
                        }
                        for (int k = 0; k < gp[5]; k++)
                        {
                            //生成planobject
                            PlanObject plan = new PlanObject();
                            plan.BatchID = B.ID.ToString();
                            ...
                            plan.Building = B.Projects[i].Buildings[j].ID.ToString();
                            plan.Floor = downf.ToString();
                            plan.ComponentType = ("异形构件");
                            plan.ComponentID = k.ToString();
                            plan.FlowLineNumber = ("固定模台生产线");
                            DateTime d = time;
                            time = time.AddHours(gp[13]);
                            plan.ActualStartTime = d;
                            plan.ActualEndTime = time;
                            list.Add(plan);
                        }
        ...
        public int[] GJHQ(int downf, BatchTask B, int pid, int bid)//获取构件算法 /*测试完毕无误*/
        {
            ...
            for (int i = 0; i < ci; i++)
            {
                if (B.Projects[pid].Buildings[bid].Components[i].Floor == downf)
                {
                    ...
                    {
                        GJ[1] = B.Projects[pid].Buildings[bid].Components[i].Amount;
                    }
                    if (B.Projects[pid].Buildings[bid].Components[i].Category == 3)
                    {
                        GJ[2] = B.Projects[pid].Buildings[bid].Components[i].Amount;
                    }
        ...
        public double[] CoreFunc(int[] GJ, double[] PP, LimitedCondition L, DateTime starttime)//底层算法 /*测试error*/
        {
            /*四种生产线对应一小时的产能*/
            double P11, P12;
            double P22;
